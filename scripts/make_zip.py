...
def  main():
  ...
  installer = "\installer\BitCrushi Installer.exe"
   
  if demo:
    installer = "\installer\BitCrushi Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\BitCrushi manual.pdf" 
  ]
  ...
  ZIPNAME = "BitCrushi-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "BitCrushi-v" + FULLVERSIONSTR + "-win-demo.zip"
